...
Stop for unresolved permission model, config migration, security, or product decisions. Preserve unrelated work and `.claude/settings.local.json`; no force-push/reset/clean/branch deletion/deploy/destructive production actions. Non-merge remote writes require explicit task authorization. Prefer targeted reads/diffs.
